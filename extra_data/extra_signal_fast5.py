...
def collect_fast5_paths(fast5_path_txt):
    """
    Read all fast5 file paths from a text file.

    Each non-empty line can be:
      1) a directory
      2) a single .fast5 file
    """
    fast5_files = []
    seen = set()

    with open(fast5_path_txt, "r", encoding="utf-8") as f:
        entries = [line.strip() for line in f if line.strip()]

    for entry in entries:
        if os.path.isfile(entry) and entry.lower().endswith(".fast5"):
            ap = os.path.abspath(entry)
            if ap not in seen:
                fast5_files.append(ap)
                seen.add(ap)
            continue
        ...
        print(f"[WARN] Skip invalid fast5 path entry: {entry}", file=sys.stderr)

    return sorted(fast5_files)
...
def process_one_fast5(f5_path, read_to_records, min_start=1500, truncate_len=None, scale=False):
    """
    Process one fast5 file once and return:
    ...
    try:
        with get_fast5_file(f5_path, "r") as reader:
            for read in reader.get_reads():
                parent_read_id = str(read.read_id)
                if parent_read_id not in read_to_records:
                    continue

                try:
                    raw = np.asarray(read.get_raw_data(scale=scale))
...
def init_h5(output_path, compression="gzip"):
    ...
    meta.create_dataset("source_fast5", shape=(0,), maxshape=(None,), dtype=str_dt, chunks=True)
...
def append_batch_to_h5(h5f, batch):
    ...
    meta["source_fast5"][n_seg_old:n_seg_old + n_seg_add] = [x["source_fast5"] for x in batch]
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Extract signal segments from fast5 using one or three TSV region files. "
            "When three TSVs are provided, they are interpreted in order: "
            "train.tsv valid.tsv test.tsv. The script traverses fast5 only once "
    ...
    parser.add_argument(
        ...
        dest="fast5_path_txt",
        required=True,
        help="Text file containing fast5 directories or fast5 file paths, one per line.",
    )
    parser.add_argument(
        "--regions_tsv",
        nargs="+",
    ...
    parser.add_argument(
        "--scale",
        action="store_true",
        help="Pass scale=True to fast5 read.get_raw_data(). Default: False",
    )
    ...
    if get_fast5_file is None:
        raise ImportError(
            "ont_fast5_api is not installed in the current environment. "
            "Please install the 'ont-fast5-api' package before running this script."
        )
    ...
    fast5_files = collect_fast5_paths(args.fast5_path_txt)
    ...
    print(f"Found fast5 files: {len(fast5_files)}")
    ...
    if len(fast5_files) == 0:
        raise RuntimeError("No fast5 files were found.")
    ...
    try:
        with ThreadPoolExecutor(max_workers=max(1, args.threads)) as ex:
            ...
            for fut in tqdm(as_completed(futures), total=len(futures), desc="Processing fast5"):
                f5_path = futures[fut]
                try:
                    hits, skipped = fut.result()
                except Exception:
                    print(f"[ERROR] Failed to process: {f5_path}", file=sys.stderr)
                    traceback.print_exc()
                    continue
                ...
                for item in hits:
                    ...
                    if record_key in written_keys:
                        duplicate_rows_by_split[split].append({
                            ...
                            "source_fast5": item["source_fast5"],
    ...
    for record_key, rec in all_rows_meta.items():
        ...
        if record_key not in seen_any_keys and record_key not in written_keys:
            missing_rows_by_split[split].append({
                ...
                "bed_name": rec["bed_name"],
                "reason": "read_not_found_in_any_fast5",
            })
    ...
    for split in tsv_by_split:
        write_report_tsv(
            ...
            ["tsv_row_idx", "parent_read_id", "bed_name", "reason", "source_fast5"],
        )
        write_report_tsv(
        ...
        write_report_tsv(
            ...
            ["tsv_row_idx", "parent_read_id", "bed_name", "source_fast5", "reason"],
        )
